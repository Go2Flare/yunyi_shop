...
func HandlerRequest(ctx *gin.Context) {
	...
	//解析出callback中的filename
	fileName := params["filename"]
	fileUrl := fmt.Sprintf("%s/%s", global.ServerConfig.OssInfo.Host, fileName)

	// verifySignature and response to client
	isVertify := utils.VerifySignature(bytePublicKey, byteMD5, byteAuthorization)
	//fmt.Println("utils.VerifySignature:", string(bytePublicKey), string(byteMD5), string(byteAuthorization))
	//TODO 验证公钥，md5，签名,这里先不校验，直接返回对象的url
	if !isVertify {
		// do something you want accoding to callback_body ...
		// TODO 后期做回调逻辑时候，在这里加上逻辑，现在只是返回了上传后的url
