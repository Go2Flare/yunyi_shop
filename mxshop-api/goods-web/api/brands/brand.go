...
func BrandList(ctx *gin.Context) {
	...
	fmt.Printf("good_srv 响应 BrandList:%v, 长度为%v\n", rsp.Data, len(rsp.Data))
	...
	//// pn := 1, psize := 10
	////获取指定页的数据，注意判断是否会超出已有的数量
	//dataLen := len(rsp.Data)
	fmt.Printf("dataLen = %v, pnInt*pSizeInt = %v", len(rsp.Data), pnInt*pSizeInt)
	//
	//switch {
	////请求的页区间下界超过数量
	//case dataLen < (pnInt-1)*pSizeInt:
	//	log.Println("BrandList，请求数据太大")
	//	ctx.JSON(http.StatusBadRequest, gin.H{
	//		"message": "请求数量太大，超过品牌数量",
	//	})
	//	return
	////请求的页区间上界超过数量,
	//case dataLen >= pnInt*pSizeInt:
	//	dataLen = pnInt*pSizeInt
	//default:
	//}
	//默认只获取到数据尾部
